...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'UniMiB':
        train_set = unimib_load_dataset(incl_xyz_accel=True, incl_rms_accel=False, incl_val_group=False,
                                        is_normalize=True, one_hot_encode=False, data_mode='Train', single_class=True,
        ...
        train_loader = data.DataLoader(train_set, batch_size=args.batch_size, num_workers=args.num_workers,
                                       shuffle=True)
        test_set = unimib_load_dataset(incl_xyz_accel=True, incl_rms_accel=False, incl_val_group=False,
                                       is_normalize=True, one_hot_encode=False, data_mode='Test', single_class=True,
                                       class_name=args.class_name)
        test_loader = data.DataLoader(
            test_set, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=True)
    elif args.dataset == 'Shenzhen':
        use_data_size = 1000
        train_set = shenzhen_dataset(
            data_size=use_data_size, data_mode='Train', verbose=False)
        train_loader = data.DataLoader(train_set, batch_size=args.batch_size, num_workers=args.num_workers,
                                       shuffle=True)
        test_set = shenzhen_dataset(
            data_size=use_data_size, data_mode='Test', verbose=False)
        test_loader = data.DataLoader(
            test_set, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=True)
    else:
        raise RuntimeError('Wrong Dataset Input!')

    print(len(train_loader))

    if args.max_iter:
        args.max_epoch = np.ceil(
            args.max_iter * args.n_critic / len(train_loader))
    ...
    if args.load_path:
        ...
        logger = create_logger(
            args.path_helper['log_path']) if args.rank == 0 else None
